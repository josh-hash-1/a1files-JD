 ...
 * -------------------------------------------------------------
 */
jd_void displayBuffer(BufferPointer ptr_Buffer);
jd_long getFileSize(jd_strg fname);
jd_intg isNumber(const jd_strg ns);
jd_void startReader(jd_strg, jd_strg, jd_intg, jd_real);
...
jd_intg main2Reader(jd_intg argc, jd_strg* argv) {
	...
	jd_strg program = argv[0];
	jd_strg input = argv[2];
	jd_intg size = 0;
	jd_real factor = 0.0f;
...
************************************************************
*/
jd_void startReader(jd_strg program, jd_strg input, jd_intg size, jd_real factor) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	jd_intg loadSize = 0;	/* the size of the file loaded in the buffer */
