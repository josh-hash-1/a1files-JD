...
jd_intg main1Coder(jd_intg argc, jd_strg* argv) {
	if (argc < 5) {
	...
	jd_strg operation = "";
	jd_strg inputFileName = "";
	jd_strg key = STR_LANGNAME;
	jd_strg outputFileName = "";
	...
	if (argc > 4) {
		...
		jd_intg size = getSizeOfFile(outputFileName);
		jd_strg output;
		if (atoi(operation) == CYPHER)
